...
def top_k_festival_alerts(alerts: list[tuple[int, str]], k: int) -> list[str]:
    ...
    Uses heapq.nsmallest which is efficient for small k.
    ...
    # nsmallest returns the k items with the smallest priority numbers
    top = heapq.nsmallest(k, alerts)  # list of (priority, title) tuples
    return [title for priority, title in top]
